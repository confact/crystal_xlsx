...
describe CrystalXlsx::Format do
  describe "#initialize" do
    ...
    it "initializes with custom values" do
      format = CrystalXlsx::Format.new(font_size: 14, font_name: "Arial", bold: true, text_color: "FFFFFF", bg_color: "000000", border: true)
      format.font_size.should eq 14
      format.font_name.should eq "Arial"
      format.bold?.should be_true
      format.text_color.should eq "FFFFFF"
  ...
  describe "#to_font_xml" do
    it "generates correct font XML" do
      format = CrystalXlsx::Format.new(bold: true, text_color: "FF0000")
      xml = XML.build_fragment do |xml|
        format.to_font_xml(xml)
      end
      xml.should match(/<font><sz val="11"\/><name val="Calibri"\/><b val="true"\/><color rgb="FFFF0000"\/><\/font>/)
    end
  end

  describe "#to_fill_xml" do
    it "generates correct fill XML when colors are specified" do
      format = CrystalXlsx::Format.new(text_color: "FF0000", bg_color: "00FF00")
      xml = XML.build_fragment do |xml|
        format.to_fill_xml(xml)
      end
      xml.should match(/<fill><patternFill patternType="solid"><fgColor rgb="FF00FF00"\/><bgColor indexed="0"\/><\/patternFill><\/fill>/)
    end
  ...
  describe "#to_xml" do
    it "generates the correct overall XML" do
      format = CrystalXlsx::Format.new(text_color: "FFFFFF", border: true)
      format.index = 2
      xml = XML.build_fragment do |xml|
        format.to_xml(xml)
      end
      xml.should match(/<xf numFmtId="0" fontId="2" fillId="2" borderId="2" applyFont="1" applyFill="1" applyBorder="1" applyAlignment="0"\/>/)
    end
  end
end
