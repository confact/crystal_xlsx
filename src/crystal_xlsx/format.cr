class CrystalXlsx::Format
  ...
  def initialize(@font_size : Int32 = 11, @font_name : String = "Calibri", @bold : Bool = false, @bg_color : String? = nil, @fill_color : String? = nil, @border : Bool = false)
  end
  ...
  def to_xml(xml)
    xml.element("xf", numFmtId: "22", fontId: index, fillId: (fill_color || bg_color) ? index : 0, borderId: "#{border ? index : 0}", applyFont: "1", applyFill: (fill_color || bg_color) ? 1 : 0, applyBorder: border ? 1 : 0, applyNumberFormat: "1")
  end
end